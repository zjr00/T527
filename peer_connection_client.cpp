...
void PeerConnectionClient::DoConnect() {
  ...
  bool ret = ConnectControlSocket();//连接套接字
  if (ret)
    state_ = SIGNING_IN;
  if (!ret) {
    callback_->OnServerConnectionFailure();
...
bool PeerConnectionClient::ConnectControlSocket() {
  RTC_DCHECK(control_socket_->GetState() == rtc::Socket::CS_CLOSED);
  printf("开始连接信令服务器\n");
  int err = control_socket_->Connect(server_address_);//发起连接
  if (err == SOCKET_ERROR) {
...
void PeerConnectionClient::OnConnect(rtc::Socket* socket) {
  ...
  //std::cout << "发送的内容"<<onconnect_data_.c_str() << std::endl;
...
void PeerConnectionClient::OnMessageFromPeer(int peer_id,
  ...
  if (message.length() == (sizeof(kByeMessage) - 1) &&message.compare(kByeMessage) == 0)//如果是bye信令
  {
    printf("接收到bye信令消息\n");
    callback_->OnPeerDisconnected(peer_id);
  } else {
    callback_->OnMessageFromPeer(peer_id, message);//处理处理offer、answer、candidate
  }
}
        ...
        if (content_length) {
          ...
          while (pos < control_data_.size()) {
            ...
            if (ParseEntry(control_data_.substr(pos, eol - pos), &name, &id,
                ...
                id != my_id_) {
              peers_[id] = name;
              callback_->OnPeerConnected(id, name);//将遍历到的客户端显示到自己的客户端界面
            }
            pos = eol + 1;
          }
        }
    ...
    if (ok) {
      ...
      if (my_id_ == static_cast<int>(peer_id)) {
        ...
        if (ParseEntry(notification_data_.substr(pos), &name, &id,
          ...
          if (connected) {
            peers_[id] = name;
            callback_->OnPeerConnected(id, name);//有新成员连接进来
          } else {
            peers_.erase(id);
            callback_->OnPeerDisconnected(id);//有成员断开连接
          }
        }
      } else {
        OnMessageFromPeer(static_cast<int>(peer_id),
                          notification_data_.substr(pos));//进行处理offer、answer、candidate，bey信令
      }
    }
